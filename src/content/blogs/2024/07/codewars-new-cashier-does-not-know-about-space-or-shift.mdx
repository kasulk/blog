...
Mit <ExternalLink href={docs.array.array}>`Array()`</ExternalLink> erstelle ich neues Array mit der Länge `numItems`. Dann setze ich an jede Stelle, das aktuelle Element der Speisekarte ein. Das Ganze wird dann mit `...` <ExternalLink href={docs.operators.spread}>gespreadet</ExternalLink>, damit wir die Elemente einzeln an das Ergebnis-Array anhängen können.
